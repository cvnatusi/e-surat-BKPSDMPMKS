    ...
    <div class="card-body">
      <div class="table-responsive">
        <button type="button" class="btn btn-primary btn-add" style="width: 170px;">
          <i class="bx bx-plus me-1"></i>Jabatan Baru</button>
        <p></p>
...
@section('js')
  ...
  $('.btn-add').click(function(){
      // $('.preloader').show();
      $('.main-page').hide();
      $.post("{!! route('form-jabatan') !!}").done(function(data){
  ...
  function editForm(id) {
    console.log(id)
    $('.main-page').hide();
      $.post("{!! route('form-jabatan') !!}",{id:id}).done(function(data){
          if(data.status == 'success'){
            $('.other-page').html(data.content).fadeIn();
          } else {
            $('.main-page').show();
   ...
   function deleteForm(id) {
     swal({
       ...
       if (result.value) {
         $.post("{!! route('destroy-jabatan') !!}",{id:id}).done(function(data){
