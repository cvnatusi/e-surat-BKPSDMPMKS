<div class="card border-top border-0 border-4 border-primary panel-form">
    <div class="card-body">
      <div class="card-title d-flex align-items-center">
        ...
        <h5 class="mb-0 text-primary">@if($data) Edit @else Tambah @endif Level Pengguna</h5>
      </div>
      <hr>
      <form class="row g-3 form-save">
        ...
        <div class="col-md-6">
          ...
          <input type="text" class="form-control" name="nama_level_pengguna" id="nama_level_pengguna" @if(!empty($data)) value="{{$data->name}}" @endif placeholder="Nama Level Pengguna">
        </div>
        <div class="col-md-6">
          <label for="singkatan" class="form-label">Singkatan</label>
          <input type="text" class="form-control" name="singkatan" id="singkatan" @if(!empty($data)) value="{{$data->singkatan}}" @endif placeholder="Singkatan">
        </div>
        <hr>
        <div class="col-md-12">
  ...
  $('.btn-submit').click(function(e){
   e.preventDefault();
      ...
      $.ajax({
          url: "{{ route('store-level-pengguna') }}",
          type: 'POST',
          data: data,
          async: true,
          cache: false,
