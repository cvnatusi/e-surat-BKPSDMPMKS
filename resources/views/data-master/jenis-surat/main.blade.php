    ...
    <div class="card-body">
      <div class="table-responsive">
        <button type="button" class="btn btn-primary btn-add" style="width: 170px;">
          <i class="bx bx-plus me-1"></i>Jenis Baru</button>
        <p></p>
        <table class="table table-striped dataTable" id="datagrid" style="width: 100%">
          <thead>
            <td>NO</td>
            <td>KODE SURAT</td>
            <td style="width: 100%">NAMA JENIS SURAT</td>
            {{-- <td>MASUK / KELUAR</td> --}}
            <td>AKSI</td>
          </thead>
        </table>
...
@section('js')
  ...
  $('.btn-add').click(function(){
      // $('.preloader').show();
      $('.main-page').hide();
      $.post("{!! route('form-jenis-surat') !!}").done(function(data){
  ...
  function editForm(id) {
    $('.main-page').hide();
      $.post("{!! route('form-jenis-surat') !!}",{id:id}).done(function(data){
          if(data.status == 'success'){
            $('.other-page').html(data.content).fadeIn();
          } else {
            $('.main-page').show();
   ...
   function deleteForm(id) {
     swal({
       ...
       if (result.value) {
         $.post("{!! route('destroy-jenis-surat') !!}",{id:id}).done(function(data){
          if (data.status == 'success') {
